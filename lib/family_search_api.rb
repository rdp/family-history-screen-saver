...
module FamilySearchApi
  ...
  def self.warmup
    @@ancestors = []
    Thread.new { give_me_all_ancestors_as_hashes(@@ancestors); puts 'done downloading ancestors' }
    while @@ancestors.length == 0
      sleep 0.01
    end
    # now at least one exists :P
  end
...
# like [{:name, :relation_level, :gender, :birth_place, :birth_year}, {:name...}, ... ]
  def self.give_me_all_ancestors_as_hashes array_to_add_to
    $com = FsCommunicator.new :domain => 'http://www.dev.usys.org', :handle_throttling => true
    ...
    add_person(starting, array_to_add_to, 0)
    array_to_add_to
  end
